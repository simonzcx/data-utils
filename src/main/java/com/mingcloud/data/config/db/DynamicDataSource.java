...
//                    throw new ADIException("不存在的数据源："+datasource,500);
    ...
    /**
     * Specify the default target DataSourceEntity, if any.
     * <p>
     ...
     * <p>
     * This DataSourceEntity will be used as target if none of the keyed
    ...
    public void createDataSourceWithCheck(DataSourceEntity dataSourceEntity) throws Exception {
        String datasourceId = dataSourceEntity.getId();
        log.info("准备创建数据源"+datasourceId);
        log.info("username:{}", dataSourceEntity.getUserName());
        log.info("password:{}", dataSourceEntity.getPassword());
        log.info("url:{}", dataSourceEntity.getUrl());
        Map<Object, Object> dynamicTargetDataSources2 = this.dynamicTargetDataSources;
        if (dynamicTargetDataSources2.containsKey(datasourceId)) {
    ...
    private  void createDataSource(DataSourceEntity dataSourceEntity) throws Exception {
        String datasourceId = dataSourceEntity.getId();
        log.info("准备创建数据源"+datasourceId);
        String databasetype = dataSourceEntity.getDatabasetype();
        ...
        if(testDatasource(datasourceId,driveClass,url,username,password)) {
            ...
            log.info("password:{}", dataSourceEntity.getPassword());
            log.info("url:{}", dataSourceEntity.getUrl());
            log.info("databasetype:{}", dataSourceEntity.getDatabasetype());
        }
    }
